...
CREATE INDEX idx_machine_reservations_machine ON machine_reservations(machine_id);
CREATE INDEX idx_machine_reservations_transaction ON machine_reservations(transaction_id);
CREATE INDEX idx_machine_reservations_time ON machine_reservations(start_time, end_time);

CREATE TABLE laundry_service_orders (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    transaction_id UUID NOT NULL REFERENCES transactions(id),
...
CREATE INDEX idx_laundry_orders_transaction ON laundry_service_orders(transaction_id);

CREATE TABLE drop_off_locker_reservations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_drop_off_locker_order ON drop_off_locker_reservations(laundry_order_id);
CREATE INDEX idx_drop_off_locker_locker ON drop_off_locker_reservations(locker_id);
